...
class SumLayers(nn.Module):
    ...
    def forward(self, X, present):
        ...
        return x.sum(1) #sums across rows
...
class GraphMatLayer(nn.Module):
    def __init__(self, input_feature_dim, output_feature_dim, GS=1,  
        ...
        GS specifies number of adjacency matrices - one for each bond order in the compound
        linlayers is a list of linear transformations, one for each bond order
        dropout_layers  is a list of dropout layers to follow linear transformations
        in each iteration, the data is passed through linear trasnformations/dropout, once for each bond order
        each resulting vector then passed through ReLU
        those vectors are passed through a max aggregation function that chooses the max value for each vector
        """
        super(GraphMatLayer, self).__init__()

        self.GS = GS #number of adjacency matrices
        self.noise=noise
...
class GraphMatLayers(nn.Module):
    def __init__(self, input_feature_n, 
        ...
        '''
        GraphMatLayer servers as basis for this class
        Data is passed through GraphMatLayer
        Resulting vector is aggregated in residual manner, so long as dimensions match
        '''
...
class GraphVertModel(nn.Module):
    def __init__(self, g_feature_n, g_feature_out_n=None, 
        ...
        """
        GraphMatLayers serves as the basis for this model
        
        """
        if layer_n is not None:
            g_feature_out_n = [int_d] * layer_n #list of int_d (2048) layer_n (10) times; this results in [2048, 2048, 2048, ... ten times]
